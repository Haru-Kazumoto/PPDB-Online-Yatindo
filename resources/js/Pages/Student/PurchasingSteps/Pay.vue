<template>
    <div class="p-2">
        ...
        <div class="row border-2 rounded border-primary p-2 mt-4  g-2">
            <div class="col-12 d-flex flex-column">
                <label for="bank">Bank</label>
                <span><b>{{ data.batches.bank_name }}</b></span>
            </div>
            <div class="col-12 d-flex flex-column">
                <label for="bank">Nominal pembayaran</label>
                <span><b>{{ formatRupiah(data.batches.registration_price) }}</b></span>
            </div>
            <div class="col-12 d-flex flex-column">
                <label for="bank">Nomor Rekening</label>
                <span><b>{{ data.batches.bank_number_account }}</b></span>
            </div>
            <div class="col-12 d-flex flex-column">
                <label for="bank">Atas Nama</label>
                <span><b>{{ data.batches.bank_username }}</b></span>
            </div>
        </div>

        <!-- Tombol Upload -->
        <button class="btn btn-md btn-primary mt-4 w-100 w-sm-auto" @click="showModal">Upload bukti bayar</button>
            ...
            <table class="table table-bordered w-100">
                ...
                <tbody align="center">
                    <tr v-for="(payment, index) in data.payments" :key="index">
                        <td>
                            <n-image width="200" :src="`/storage/${payment.payment_image}`" />
                        </td>
        ...
        <!-- Tombol Lanjutkan -->
        <div class="d-flex justify-content-center" v-if="data.payments.length != 0">
            <button @click="handleAssign()" class="btn btn-primary w-100 w-sm-auto">Lanjutkan</button>
        </div>
    </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body d-flex flex-column gap-2">
                    ...
                    <!-- Menampilkan input tambahan hanya jika metode pembayaran adalah transfer -->
...
import { formatRupiah } from '../../../Utils/menus';
